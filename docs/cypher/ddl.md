...
```
The above ddl indicates that LivesIn has 1-to-n multiplicity. This ddl command puts the constraint: that each User node v might be Liked by one node (either User or Admin). Now, it does not put any constraint in the forward direction, i.e., each User node (or Animal) node might know multiple Users.
